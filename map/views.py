...
from timeSeries.satelliteData import TRMMSatelliteRainfall
...
def mainMap(request):
    ...
    for r0 in rasters:
        ...
        rasterObj = eval(raster + '(dataFolder=dataFolder, downloadFolder=downloadFolder)')
        tmp = []
        ...
        if len(tmp)>0:
            ...
            rasterDict[r0.name]['reference'] = r0.startDate.astimezone(pytz.timezone(settings.TIME_ZONE)).replace(tzinfo=None).isoformat() + 'Z'
