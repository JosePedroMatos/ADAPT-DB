...
function requestRasterData(name) {
	...
	var range = 15*86400*1000;
	var tmpDateIni = new Date($("#dateSlider").slider("value")).getTime()-range;
	var tmpDateEnd = new Date($("#dateSlider").slider("value")).getTime()+range;
...
function createWebGLSquares(lat, lon, idxs) {
	var dLat = (lat[1]-lat[0])/2;
	var dLon = (lon[1]-lon[0])/2;
