...
class errorMetrics(object):
    ...
    def __init__(self, targets, stride=100, workGroup=(16, 16), platform=0, deviceType='ALL', verbose=0):
    ...
    def _prepOpenCL(self):
        ...
        kernelStr=pkg_resources.resource_string(__name__, 'eval.cl')#@UndefinedVariable
    ...
    def reshapeData(self, simulations):
        ...
        #=======================================================================
        # self.targets=np.hstack((self.targets, np.zeros(self.sizes['addObs'])))
        # self.toAddRight=np.zeros((self.sizes['reshapedObs'], self.sizes['addPop']))
        #=======================================================================
    ...
    def compute(self, simulations):
        ...
        kernel(self.openCL.queue, globalSize, localSize,
               ...
               #================================================================
               # cl.LocalMemory(np.int32(1).nbytes*self.stride*self.openCL.workGroup[0]))
               #================================================================
        
        error = np.empty((np.prod(globalSize),)).astype(np.float32)
        cl.enqueue_copy(self.openCL.queue, error, outErrorBuffer)
        error=np.reshape(error, globalSize, order='F')[:self.sizes['reshapedObs']/self.stride,:self.sizes['originalPop']]
        mse=np.sum(error,0)/self.sizes['originalObs']
        ...
        nonExceedance=np.reshape(nonExceedance, globalSize, order='F')[:self.sizes['reshapedObs']/self.stride,:self.sizes['originalPop']]
        nonExceedanceFraction=(np.sum(nonExceedance,0))/self.sizes['originalObs']

        return (mse, nonExceedanceFraction)
