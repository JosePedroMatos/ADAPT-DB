...
class PSOAlt(OptAlgorithm):
    ...
    def newPop(self, fit, population):
        ...
        tmpRnd1=self.opt['c1']*np.random.uniform(low=0.0, high=1.0, size=(self.opt['population'],self.opt['nVars']))
        tmpRnd2=self.opt['c2']*np.random.uniform(low=0.0, high=1.0, size=(self.opt['population'],self.opt['nVars']))
        candidateVelocities=(self.opt['inertia']+np.random.uniform(low=0, high=0.5, size=(self.opt['population'], self.opt['nVars'])))*self.velocities + tmpRnd1 * (pBest-self.population) + tmpRnd2 * (gBest-self.population)
             
        candidateVelocities*=np.random.uniform(low=-1.0, high=1.0, size=(self.opt['population'],1)).repeat(self.opt['nVars'],1)
